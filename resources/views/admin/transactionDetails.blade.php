...
<head>
    ...
    <script>
        fetch('/api/transactiondetails')
            .then(response => response.json())
            .then((data) => {
                (Array.from(data)).forEach((element) => {
                    const mr = document.querySelector('#Show')
                    let el = document.createElement('tr');
                    el.innerHTML =
                        `<td>${element.bill_number}</td> <td>${element.user_id}</td> <td>${element.anount}</td> <td>${element.address_id}</td>`
                    mr.appendChild(el)
                })
            });
    </script>
